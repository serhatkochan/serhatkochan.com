      ...
      <article className="md:grid md:grid-cols-4 md:items-baseline">
        <Card className="md:col-span-3">
          ...
          >
            ...
            {note.inProgress && <StaticBadge className="ml-4">Devam ediyor</StaticBadge>}
          </Card.Eyebrow>
          <Card.Description>{note.description}</Card.Description>
          <Card.Cta>Notu okuyun</Card.Cta>
        </Card>
        {!dense && (
          <Card.Eyebrow as="time" dateTime={note.publishedAt} className="mt-1 hidden md:block">
            {formatDate(note.publishedAt)}
            <br/>
            {note.inProgress && <StaticBadge className="mt-2">Devam ediyor</StaticBadge>}
          </Card.Eyebrow>
        )}
      </article>
    </motion.div>
